...
const log = bunyan.createLogger({name: "models/article"});
...
class Article {
    constructor(planet, param) {
        ...
        this.articlePath = require('path').join(planet.articlesPath, `${
            this.id
        }.json`)
    ...
    savePublic() {
        require('fs').writeFileSync(this.publicArticlePath, JSON.stringify({
            "attachments": [
