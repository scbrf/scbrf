...
class FollowingPlanet {
    static following = []
    static followingPlanetsPath = require('path').join(__dirname, '..', '..', '..', 'Following')
    constructor(params) {
    ...
    static startUpdate() {
        setInterval(() => {
            for (let planet of FollowingPlanet.following) {
                planet.update()
            }
    ...
    static async followIPNSorDNSLink(ipns, cb) {
        ...
        planet.articles = publicPlanet.articles.map(a => FollowingArticle.create({
            ...a,
