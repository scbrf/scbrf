...
class Planet {
    static planets = []
    static myPlanetsPath = require('path').join(__dirname, '..', '..', '..', 'My')
    ...
    savePublic() {
        ...
        require('fs').writeFileSync(this.publicInfoPath, JSON.stringify({
            about: this.about,
    ...
    async publish() {
        ...
        await Promise.all(this.articles.map(a => async () => {
            await a.savePublic()
