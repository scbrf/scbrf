...
const log = bunyan.createLogger({name: "editorwebview"});
...
class EditorTopbar {
    ...
    async runJS(func) {
        const code = "(" + func + `)(${
            [...arguments].slice(1).map((s) => JSON.stringify(s)).join(",")
        })`;
    ...
    async updateWebviewReal(p) {
        ...
        return await this.runJS((html) => {
            ...
            for (let i = 0; i < div.childNodes.length; i++) { // 如果当前节点一样，直接跳过去处理下一个节点
                ...
                for (let target = idx + 1; target < document.body.childNodes.length; target++) {
                    if (document.body.childNodes[target].outerHTML === div.childNodes[i].outerHTML) { // 如果找到了，就把前面的节点都删掉
