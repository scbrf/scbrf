...
class IPFSDaemon { // 启动并监视IPFS Daemon
    ...
    async launchDaemon() {
        this.daemon = spawn(IPFSDaemon.EXE_PATH, ["daemon", "--migrate", "--enable-namesys-pubsub", "--enable-pubsub-experiment"]);
    ...
    async publish(key, cid) {
        ...
        const rsp = await this.api('name/publish', {
            "arg": cid,
            "allow-offline": "1",
            "key": key,
            "quieter": "1",
