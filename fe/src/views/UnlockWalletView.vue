<template>
    <div class="p-4 w-screen h-screen drag flex flex-col">
        ...
        <label class="input-group input-group-sm w-100 flex justify-center mt-2 nodrag">
            <input @keyup.enter="unlockWallet" :disabled="disabled" type="password" placeholder="请输入密码!"
                v-model="passwd" class="input input-sm input-bordered font-bold text-center" autofocus />
        </label>
        ...
        <div class="flex justify-center m-2">
            <button class="btn btn-sm btn-outline nodrag" :disabled="!validate" @click="unlockWallet">解锁钱包</button>
        </div>
    </div>
</template>
<script>
